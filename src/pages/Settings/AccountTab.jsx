...
import { selectCurrentUser, updateUserInfoApi } from '~/redux/User/userSlide'
...
function AccountTab() {
  ...
  const submitChangeGeneralInformation = (data) => {
    ...
    // Gọi API...
    toast.promise(dispatch(updateUserInfoApi({ displayName })), {
      pending: 'Updating...'
    }).then(res => {
      // Nếu thành công thì cập nhật lại giá trị trong form
      if (!res.error) {
  ...
  const uploadAvatar = (e) => {
    ...
    // Gọi API...
    toast.promise(dispatch(updateUserInfoApi(reqData)), {
      pending: 'Updating...'
    }).then(res => {
      // Nếu thành công thì cập nhật lại giá trị trong form
      // lưu ý dù api gọi có lỗi hãy không thì chúng ta phải clear input file để người dùng có thể chọn lại file khác
      e.target.value = ''
    })
  }
