...
function AutoCompleteSearchBoard() {
  ...
  const handleInputSearchChange = (event) => {
    ...
    setLoading(true)
    fetchBoardsApi(searchPath).then((response) => {
  ...
  // Làm useDebounceFn...
  const handleInputSearchChangeDebounced = useDebounceFn(handleInputSearchChange, 1000)
  ...
  const handleSelectedBoard = (event, selectedBoard) => {
    ...
    if (selectedBoard) {
      ...
      // Đóng popup suggestions và clear input sau khi hoàn thành tìm kiếm
      setOpen(false)
      event.target.value = ''
    }
  }
  ...
  return (
    <Autocomplete
      ...
      onInputChange={handleInputSearchChangeDebounced}
